...
int main(int argc, char **argv) {
  ...
  *d_OutputGPU;
  ...
  //Device Memory
  cudaMalloc(&d_Input, imageW * imageH * sizeof(double));
  cudaMalloc(&d_OutputGPU, imageW * imageH * sizeof(double));
  ...
  if(!h_Filter || !d_Input || !h_Buffer || !h_OutputCPU || !d_OutputGPU || !h_OutputGPU)
    return -1;
  ...
  //Kernel call
  convolutionRowGPU<<<grid_dimensions, block_dimensions, shared_memory_size_horizontal * sizeof(double)>>>(d_Buffer, d_Input, imageW, imageH, filter_radius); //First Kernel must finish copying data
  convolutionColumnGPU<<<grid_dimensions, block_dimensions, shared_memory_size_vertical * sizeof(double)>>>(d_OutputGPU, d_Buffer, imageW, imageH, filter_radius); //After copying is finished launch
