...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (i = 1; i < size_invariant; i+=2) {
		output[i*SIZE] = 0;
		output[i*SIZE + size_invariant] = 0;
		output[(i+1)*SIZE] = 0;
		output[(i+1)*SIZE + size_invariant] = 0;
	}
	...
	for (i=1; i<size_invariant; i+=1) {
		for (j=1; j< size_invariant; j+=2) {
			...
			//output[i][j]
			p = pow(input[(i - 1)*SIZE + j + -1] * horiz_operator[0][0] +
				input[(i - 1)*SIZE + j] * horiz_operator[0][1] +
				input[(i - 1)*SIZE + j + 1] * horiz_operator[0][2] +
				input[(i)*SIZE + j - 1] * horiz_operator[1][0] +
				input[(i)*SIZE + j] * horiz_operator[1][1] +
				input[(i)*SIZE + j + 1] * horiz_operator[1][2] +
				input[(i + 1)*SIZE + j - 1] * horiz_operator[2][0] +
				input[(i + 1)*SIZE + j] * horiz_operator[2][1] +
				input[(i + 1)*SIZE + j + 1] * horiz_operator[2][2], 2) +
				pow(input[(i - 1)*SIZE + j + -1] * vert_operator[0][0] +
				input[(i - 1)*SIZE + j] * vert_operator[0][1] +
				input[(i - 1)*SIZE + j + 1] * vert_operator[0][2] +
				input[(i)*SIZE + j - 1] * vert_operator[1][0] +
				input[(i)*SIZE + j] * vert_operator[1][1] +
				input[(i)*SIZE + j + 1] * vert_operator[1][2] +
				input[(i + 1)*SIZE + j - 1] * vert_operator[2][0] +
				input[(i + 1)*SIZE + j] * vert_operator[2][1] +
				input[(i + 1)*SIZE + j + 1] * vert_operator[2][2], 2);
			...
			//output[i][j+1]
			p = pow(input[(i - 1)*SIZE + j] * horiz_operator[0][0] +
				input[(i - 1)*SIZE + j+1] * horiz_operator[0][1] +
				input[(i - 1)*SIZE + j+2] * horiz_operator[0][2] +
				input[(i)*SIZE + j] * horiz_operator[1][0] +
				input[(i)*SIZE + j+1] * horiz_operator[1][1] +
				input[(i)*SIZE + j+2] * horiz_operator[1][2] +
				input[(i + 1)*SIZE + j] * horiz_operator[2][0] +
				input[(i + 1)*SIZE + j+1] * horiz_operator[2][1] +
				input[(i + 1)*SIZE + j+2] * horiz_operator[2][2], 2) +
				pow(input[(i - 1)*SIZE + j] * vert_operator[0][0] +
				input[(i - 1)*SIZE + j+1] * vert_operator[0][1] +
				input[(i - 1)*SIZE + j+2] * vert_operator[0][2] +
				input[(i)*SIZE + j] * vert_operator[1][0] +
				input[(i)*SIZE + j+1] * vert_operator[1][1] +
				input[(i)*SIZE + j+2] * vert_operator[1][2] +
				input[(i + 1)*SIZE + j] * vert_operator[2][0] +
				input[(i + 1)*SIZE + j+1] * vert_operator[2][1] +
				input[(i + 1)*SIZE + j+2] * vert_operator[2][2], 2);
			res = (int)sqrt(p);
			/* If the resulting value is greater than 255, clip it *
			 * to 255.											   */
			if (res > 255)
