...
void gpu_equalization(int *hist_out, unsigned char *img_in, unsigned char * img_out, int img_size, int nbr_bin){
    ...
    tiles = (unsigned char **) malloc(TILES * sizeof(unsigned char *));
    if(!tiles){
        exit(EXIT_FAILURE);
    }

    // Histogram array of pointers to hold device's histograms for each tile
    histograms = (int **) malloc(TILES * sizeof(int *));
	if(!histograms){
		exit(EXIT_FAILURE);
	}	
    ...
    for(i = 0; i < TILES; i++){
        cudaMallocManaged((unsigned char **) &tiles[i], tile_size * sizeof(unsigned char));
        if(!tiles[i])
            exit(EXIT_FAILURE);
        ...
        cudaMallocManaged((int **) &histograms[i], nbr_bin * sizeof(int));
		if(!histograms[i]){
			exit(EXIT_FAILURE);
		}
...
#pragma omp parallel
{
    ...
    for(tile_index = 0; tile_index < TILES; tile_index += STREAMS){
        ...
        for(stream_index = 0; stream_index < STREAMS; stream_index++){
            ...
            // Copy tiles to unified memory
            cudaMemcpyAsync(tiles[tile_index + stream_index], &img_in[(tile_index + stream_index) * tile_size], size, cudaMemcpyHostToDevice, streams[stream_index]);
			cudaCheckError();
            
            // Kernel Call
			calculate_histogram<<<grid_dimensions, block_dimensions, shared_memory_size, streams[stream_index]>>>(tiles[tile_index + stream_index], histograms[tile_index + stream_index], size);
			cudaCheckError();
...
#pragma omp parallel
{
    ...
    for(tile_index = 0; tile_index < TILES; tile_index += STREAMS){
		...
		for(stream_index = 0; stream_index < STREAMS; stream_index++){
			...
			equalize_image<<<grid_dimensions, block_dimensions, shared_memory_size, streams[stream_index]>>>(tiles[tile_index + stream_index], tile_size);
			cudaCheckError();
            ...
            cudaMemcpyAsync(&img_out[(tile_index + stream_index) * tile_size], tiles[tile_index + stream_index], size * sizeof(unsigned char), cudaMemcpyDeviceToHost, streams[stream_index]);
			cudaCheckError();
